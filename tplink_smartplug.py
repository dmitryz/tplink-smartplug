...
parser.add_argument('--influxdb_voltage', type=str, metavar="query", default=None,
		help='query to store voltage as Volts, e.g. voltage,type=elec,device=hs110-1 this will be appended with <voltage in V> (as float)')
...
if (args.command == "energy") and (args.influxdb != None):
	...
	if (args.influxdb_voltage != None):
		post_data += "\n"+args.influxdb_voltage+" value="+str(voltage_V)
